...
func newTestService(t *testing.T) (*registry.Service, *pg.Client) {
	...
	if err := client.WithConn(ctx, func(ctx context.Context, conn pg.Querier) error {
		_, err := conn.Exec(ctx, `TRUNCATE identities, memberships, frameworks, framework_versions, controls, approvals, distribution_tokens, signing_keys, audit_log, downloads, distribution_events, mapping_sets, qa_templates CASCADE;`)
		return err
	}); err != nil {
