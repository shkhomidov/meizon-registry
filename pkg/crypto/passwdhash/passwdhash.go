...
// Package passwdhash hashes identity passwords with PBKDF2-HMAC-SHA256 over a
// per-deployment server-side pepper. Hashes are stored as a self-describing
// binary blob so the iteration count and salt travel with the digest. Plaintext
...
// NewProfile builds a hashing profile. The pepper must be at least 32 bytes and
// the iteration count at or above the security minimum.
...
// HashPassword returns the encoded hash of password. Layout:
// [1B version][1B algorithm][4B iterations][1B salt length][salt][hash].
func (hp Profile) HashPassword(password []byte) ([]byte, error) {
	...
	peppered := hp.applyPepper(password)
	hash := pbkdf2.Key(peppered, salt, int(hp.iterations), int(hp.keyLength), sha256.New)
...
// ComparePasswordAndHash reports whether password matches the encoded hash,
...
func (hp Profile) ComparePasswordAndHash(password, passwordHash []byte) (bool, error) {
	...
	peppered := hp.applyPepper(password)
	newHash := pbkdf2.Key(peppered, salt, int(iterations), len(storedHash), sha256.New)
