// Canonical region vocabulary for the registry.
//
// The stored value is always the CODE (e.g. "GLOBAL", "EUROPE", "UZ") — this is
// the format persisted on frameworks and on role/token region scopes, and the
// one the IAM region check matches on. "GLOBAL" is the wildcard: a principal
// scoped to GLOBAL may act in any region.
//
// Ordering is deliberate: global first, then continents, then countries — so a
// user picks the broadest scope that fits without scrolling a country list.

export const REGION_GLOBAL = 'GLOBAL'

export const CONTINENTS = [
  { code: 'GLOBAL', name: 'Global (all regions)' },
...
// Economic / political blocs that standards commonly scope to. Kept separate
// from continents because they are not geographic.
export const BLOCS = [
...
// Groups in picker order: continents, then blocs, then countries.
export const REGION_GROUPS = [
  { label: 'Continents', options: CONTINENTS },
  { label: 'Unions & blocs', options: BLOCS },
  { label: 'Countries', options: COUNTRIES },
]

const ALL = [...CONTINENTS, ...BLOCS, ...COUNTRIES]
const BY_CODE = new Map(ALL.map((r) => [r.code, r.name]))

export function isKnownRegion(code) {
  return BY_CODE.has(code)
}

// regionLabel renders "Name (CODE)"; unknown/legacy codes pass through unchanged
// so historical values keep displaying rather than silently disappearing.
export function regionLabel(code) {
  if (!code) return ''
  const name = BY_CODE.get(code)
